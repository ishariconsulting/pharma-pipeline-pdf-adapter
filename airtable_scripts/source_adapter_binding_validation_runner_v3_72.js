/*
V3.72 — SOURCE WATCH ADAPTER BINDING VALIDATION RUNNER + STATIC PIPELINE QA
Supported adapters: GENERIC_HTML_CONTENT_V1 + PIPELINE_GENERIC_HTML_V1 + PIPELINE_GENERIC_XLSX_V1 + PIPELINE_GENERIC_PDF_TABLE_V1 + PIPELINE_LILLY_INVESTOR_PDF_V1
...
async function fetchGenericXlsxPipeline(url, companyName) {
    if (!adapterApiKey) throw new Error("PIPELINE_GENERIC_XLSX_V1 requires Airtable secret adapterApiKey.");
    const endpoint = EXTERNAL_ADAPTER_BASE_URL +
        "/extract/generic/xlsx-pipeline?company=" + encodeURIComponent(companyName) +
        "&source_url=" + encodeURIComponent(url);
    const response = await fetch(endpoint, {
        method: "GET",
        headers: {"x-adapter-key": adapterApiKey, "Accept": "application/json"}
    });
    const bodyText = await response.text();
    let body;
    try { body = JSON.parse(bodyText); }
    catch (_) { throw new Error("Semantic XLSX adapter returned non-JSON HTTP " + response.status + "."); }
    if (!response.ok) {
        const detail = body && body.detail ? JSON.stringify(body.detail) : "HTTP " + response.status;
        throw new Error("Semantic XLSX adapter failed: " + detail);
    }
    return body;
}
async function fetchStaticPipeline(url, companyName, adapterName) {
    if (!adapterApiKey) {
        throw new Error(adapterName + " requires Airtable secret adapterApiKey.");
    }

    let endpointPath;
    let query = "";

    if (adapterName === "PIPELINE_GENERIC_PDF_TABLE_V1") {
        endpointPath = "/extract/generic/pdf-pipeline-table";
        query = "?company=" + encodeURIComponent(companyName) +
            "&source_url=" + encodeURIComponent(url);
    } else if (adapterName === "PIPELINE_LILLY_INVESTOR_PDF_V1") {
        endpointPath = "/extract/lilly/pipeline";
    } else {
        throw new Error("Unsupported static pipeline adapter: " + adapterName);
    }

    const response = await fetch(EXTERNAL_ADAPTER_BASE_URL + endpointPath + query, {
        method: "GET",
        headers: {"x-adapter-key": adapterApiKey, "Accept": "application/json"}
    });
    ...
    try { body = JSON.parse(bodyText); }
        ...
        throw new Error(
            adapterName + " returned non-JSON HTTP " + response.status + "."
        );
    }

    if (!response.ok) {
        const detail = body && body.detail ? JSON.stringify(body.detail) : "HTTP " + response.status;
        throw new Error(adapterName + " failed: " + detail);
    }
    return body;
}
...
async function writeRun({
    ...
    return await validationRuns.createRecordAsync({
        ...
        [VF.runNotes]:
            "V3.72 source-specific Adapter Binding QA. Adds reusable semantic XLSX/PDF and Lilly static-pipeline validation while preserving HTML behaviour. Static validation is source-structure based and does not require an existing Portfolio alias. Uses Reconciliation validation type for compatibility with existing select options. No master intelligence writes.",
    });
}
...
if (!["HTML Page", "PDF / Document"].includes(fetchMethod)) {
    issues.push(`Fetch Method ${fetchMethod || "blank"} is not supported by this binding runner.`);
}
...
const adapterNameAllowed =
    (sourceType === "Pipeline Page" && [
        "PIPELINE_GENERIC_HTML_V1",
        "PIPELINE_GENERIC_XLSX_V1",
        "PIPELINE_GENERIC_PDF_TABLE_V1",
        "PIPELINE_LILLY_INVESTOR_PDF_V1"
...
const staticPipelineAdapters = new Set([
    "PIPELINE_GENERIC_XLSX_V1",
    "PIPELINE_GENERIC_PDF_TABLE_V1",
    "PIPELINE_LILLY_INVESTOR_PDF_V1"
]);
if (staticPipelineAdapters.has(adapterName) && fetchMethod !== "PDF / Document") {
    issues.push(`${adapterName} requires Fetch Method PDF / Document; current=${fetchMethod || "blank"}.`);
}
if (!staticPipelineAdapters.has(adapterName) && fetchMethod !== "HTML Page") {
    issues.push(`HTML adapter ${adapterName || "unresolved"} requires Fetch Method HTML Page; current=${fetchMethod || "blank"}.`);
}
...
if (issues.length === 0) {
    try {
        let handledByStaticAdapter = false;

        if (adapterName === "PIPELINE_GENERIC_XLSX_V1") {
            const companyName = companyNames[0] || "";
            const extracted = await fetchGenericXlsxPipeline(url, companyName);
            const rows = Array.isArray(extracted.rows) ? extracted.rows : [];
            const adapterIssues = Array.isArray(extracted.issues) ? extracted.issues : [];
            const summary = extracted.summary || {};
            const diagnostics = extracted.diagnostics || {};

            retrievalMode = extracted.retrievalMode || "STATIC_DOCUMENT";
            httpStatus = 200;
            finalUrl = extracted.finalUrl || url;
            contentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
            title = companyName + " pipeline workbook";
            visibleText = rows.length + " structured pipeline rows";
            fingerprint = fnv1a(JSON.stringify(rows.map(r => [r.developmentCode, r.molecule, r.brand, r.indication, r.phase])));
            evidence = rows.slice(0, 60).map((r, idx) => ({
                text: [r.developmentCode, r.molecule, r.brand, r.indication, r.phase].filter(Boolean).join(" | "),
                url: finalUrl,
                score: 10,
                row: idx + 1
            }));
            ...
            const structuralPass =
                extracted.readyForDiscovery === true &&
                summary.structuralValidationPass === true &&
                adapterIssues.length === 0 &&
                rows.length >= 4 &&
                rows.every(r => r && r.asset && r.indication && r.phase) &&
                Number(diagnostics.rowFailures || 0) === 0 &&
                Number(diagnostics.phaseUnresolved || 0) === 0 &&
                Number(diagnostics.boundaryWarnings || 0) === 0 &&
                Number(diagnostics.exactDuplicates || 0) === 0;

            if (!structuralPass) {
                issues.push("Semantic XLSX structural validation failed. rows=" + rows.length + "; adapterIssues=" + JSON.stringify(adapterIssues).slice(0, 1200));
            }

            pipelineDiagnostics = {
                adapterVersion: extracted.version || "",
                routeVersion: extracted.routeVersion || "",
                rowCount: rows.length,
                selectedMethod: summary.selectedMethod || "SEMANTIC_XLSX_TABLE",
                portfolioDependentValidation: summary.portfolioDependentValidation === true,
                documentBytes: Number(diagnostics.documentBytes || 0),
                sheetCount: Number(diagnostics.sheetCount || 0),
                exactDuplicatesRemoved: Number(diagnostics.exactDuplicatesRemoved || 0),
                phaseUnresolved: Number(diagnostics.phaseUnresolved || 0),
                sourceStructurePass: structuralPass
            };

            reconciliation = {
                companyIds,
                companyNames,
                routeTargets: ["Portfolio", "Intelligence Update Queue"],
                candidateCount: rows.length,
                resolvedCandidateCount: null,
                pipelineDiagnostics,
                sampleCandidates: rows.slice(0, 10).map(r => ({
                    asset: r.asset, molecule: r.molecule, developmentCode: r.developmentCode,
                    brand: r.brand, indication: r.indication, phase: r.phase,
                    therapeuticArea: r.therapeuticArea
                })),
                sampleResolved: []
            };
            handledByStaticAdapter = true;
        } else if ([
            "PIPELINE_GENERIC_PDF_TABLE_V1",
            "PIPELINE_LILLY_INVESTOR_PDF_V1"
        ].includes(adapterName)) {
            const companyName = companyNames[0] || "";
            const extracted = await fetchStaticPipeline(url, companyName, adapterName);
            const rows = Array.isArray(extracted.rows) ? extracted.rows : [];
            const adapterIssues = Array.isArray(extracted.issues) ? extracted.issues : [];
            const summary = extracted.summary || {};
            const diagnostics = extracted.diagnostics || {};
            ...
            retrievalMode = extracted.retrievalMode || "STATIC_DOCUMENT";
            httpStatus = 200;
            finalUrl = extracted.finalUrl || extracted.sourceUrl || url;
            contentType = "application/pdf";
            title = companyName + " pipeline document";
            visibleText = rows.length + " structured pipeline rows";
            fingerprint = fnv1a(JSON.stringify(
                rows.map(r => [r.developmentCode, r.molecule, r.brand, r.indication, r.phase])
            ));
            evidence = rows.slice(0, 60).map((r, idx) => ({
                text: [r.developmentCode, r.molecule, r.brand, r.indication, r.phase]
                    .filter(Boolean).join(" | "),
                url: finalUrl,
                score: 10,
                row: idx + 1
            ...
            const activeRows = rows.filter(r => {
                const phase = normalizeAlias(r?.phase || "");
                return phase !== "approved" && phase !== "commercial";
            });
            const structuralPass =
                extracted.readyForDiscovery === true &&
                summary.structuralValidationPass === true &&
                adapterIssues.length === 0 &&
                activeRows.length >= 4 &&
                activeRows.every(r => r && r.asset && r.indication && r.phase) &&
                Number(diagnostics.rowFailures || 0) === 0 &&
                Number(diagnostics.phaseUnresolved || 0) === 0 &&
                Number(diagnostics.boundaryWarnings || 0) === 0 &&
                Number(diagnostics.exactDuplicates || 0) === 0;
            ...
            if (!structuralPass) {
                issues.push(
                    "Static PDF structural validation failed. rows=" + rows.length +
                    "; activeRows=" + activeRows.length +
                    "; adapterIssues=" + JSON.stringify(adapterIssues).slice(0, 1200)
                );
            }
            ...
            pipelineDiagnostics = {
                ...
                routeVersion: extracted.routeVersion || "",
                rowCount: rows.length,
                activeDevelopmentRowCount: activeRows.length,
                outOfScopeApprovedRows: rows.length - activeRows.length,
                selectedMethod: summary.selectedMethod || "",
                portfolioDependentValidation:
                    summary.portfolioDependentValidation === true,
                documentBytes: Number(diagnostics.documentBytes || 0),
                pageCount: Number(diagnostics.pageCount || 0),
                semanticTablePages: Number(diagnostics.semanticTablePages || 0),
                semanticStageBarPages: Number(diagnostics.semanticStageBarPages || 0),
                exactDuplicatesRemoved:
                    Number(diagnostics.exactDuplicatesRemoved || 0),
                phaseUnresolved: Number(diagnostics.phaseUnresolved || 0),
                rowFailures: Number(diagnostics.rowFailures || 0),
                sourceStructurePass: structuralPass
            };
            ...
            reconciliation = {
                ...
                resolvedCandidateCount: null,
                pipelineDiagnostics,
                sampleCandidates: activeRows.slice(0, 10).map(r => ({
                    asset: r.asset,
                    molecule: r.molecule,
                    developmentCode: r.developmentCode,
                    brand: r.brand,
