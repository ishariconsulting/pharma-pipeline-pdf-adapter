/*
            V2.61.0 SOURCE-WATCH-DRIVEN REGISTRY-CONTRACT PIPELINE MONITOR + PER-SOURCE FAIL-CLOSED ISOLATION + QUEUE ROUTER
            ...
            -------------------------------
            Input:
            sourceWatchRecordId = current Source Watch record from the repeating group
            ...
            - Source counts are dynamic; no company-specific historical row count is a production requirement.
            ...
            */
            ...
            const VERSION = "V2.61.0 REGISTRY-CONTRACT RECURRING PIPELINE DELTA ENGINE - PER-SOURCE FAIL-CLOSED ISOLATION + CONTROLLED QUEUE WRITES";
            ...
            function stageRank(v) {
                ...
                if (s.includes("filed") || s.includes("registration")) return 4;
            ...
            if (!norm(sourceWatchName).includes("pipeline")) {
                throw new Error(
                    ...
                    `This V2.60.0 monitor only processes validated pipeline sources.`
                );
            }
            ...
            // Registry-contract routing. New read-only pipeline adapters that expose
            // the shared INTEL_ADAPTER_V1 response contract do not require another
            // orchestration code branch. The Registry remains the allowlist: the
            // profile must be Validated, the handler must be an /extract/ route,
            // and the Source Watch retrieval mode must be explicitly supported.
            const specialisedNoQueryHandlers = new Set([
                "/extract/pfizer",
                "/extract/sanofi",
            ]);
            const specialisedSourceUrlOnlyHandlers = new Set([
                "/extract-bms-v4",
            ]);

            let expectsCommonContract = false;

            if (specialisedNoQueryHandlers.has(adapterHandlerPath)) {
                // Existing specialised endpoints require no source URL query parameter.
            } else if (specialisedSourceUrlOnlyHandlers.has(adapterHandlerPath)) {
                adapterRequestPath =
                    `${adapterHandlerPath}?source_url=${encodeURIComponent(sourceUrl)}`;
            } else if (adapterHandlerPath.startsWith("/extract/")) {
                expectsCommonContract = true;
                const joiner = adapterHandlerPath.includes("?") ? "&" : "?";
                adapterRequestPath =
                    `${adapterHandlerPath}${joiner}company=${encodeURIComponent(companyName)}&source_url=${encodeURIComponent(sourceUrl)}`;
            } else {
                throw new Error(
                    `Validated adapter "${adapterProfile}" uses handler "${adapterHandlerPath}" outside the approved /extract/ namespace. ` +
                    `Failing closed.`
                );
            }
            ...
            let external;
            try {
                external = await getJson(adapterRequestPath);
            } catch (error) {
                const holdDate = new Date().toISOString().slice(0, 10);
                const retryDate = isoDatePlusDays(holdDate, 1);
                const lastCheckedField = sourceWatchTable.getField("Last Checked");
                const nextCheckField = sourceWatchTable.getField("Next Check");
                const stateNotesField = sourceWatchTable.getField("Last Known State / Notes");

                await sourceWatchTable.updateRecordAsync(sourceWatchRecord.id, {
                    [lastCheckedField.id]: holdDate,
                    [nextCheckField.id]: retryDate,
                    [stateNotesField.id]:
                        `${VERSION}: SOURCE HOLD - adapter/source retrieval failed for ${adapterProfile}. ` +
                        `No Portfolio or queue writes were applied. Retry scheduled ${retryDate}. ` +
                        `Error: ${clean(error && error.message ? error.message : error).slice(0, 1200)}`
                });
                ...
                output.set("genericDeltaSummary", JSON.stringify({
                    ...
                    writesApplied: false,
                    status: "SOURCE HOLD - adapter/source retrieval failed; iteration isolated",
                    error: clean(error && error.message ? error.message : error).slice(0, 1200),
                    retryDate,
                    masterDataWrites: 0
                }, null, 2));
            ...
            const counts = {"Phase 1":0,"Phase 2":0,"Phase 3":0,"Filed / Registration":0,"Approved":0};
            ...
            const routeReady = expectsCommonContract
                ? external.readyForDiscovery === true
                : true;

            // Common-contract routes must prove the shared structural contract.
            // Older specialised routes retain their existing compatibility while
            // still being held to summary/diagnostic safety checks when provided.
            const structuralSummaryPass = expectsCommonContract
                ? externalSummary.structuralValidationPass === true
                : externalSummary.structuralValidationPass !== false;

            const guardrail =
                routeReady &&
                structuralSummaryPass &&
                externalIssues.length === 0 &&
                sourceRows.length >= 1 &&
                sourceRows.every(r => r && clean(r.asset) && clean(r.indication) && clean(r.phase)) &&
                ...
                Number(externalDiagnostics.coverageWarnings || 0) === 0 &&
            ...
            if (!guardrail) {
                ...
                const retryDate = isoDatePlusDays(holdDate, 1);
                await sourceWatchTable.updateRecordAsync(sourceWatchRecord.id, {
                    [sourceWatchTable.getField("Last Checked").id]: holdDate,
                    [sourceWatchTable.getField("Next Check").id]: retryDate,
                    [sourceWatchTable.getField("Last Known State / Notes").id]:
                        `${VERSION}: SOURCE HOLD - structural guardrail failed for ${adapterProfile}; ` +
                        `${sourceRows.length} source rows returned. No Portfolio or queue writes were applied. ` +
                        `Retry scheduled ${retryDate}. Issues: ${JSON.stringify(externalIssues).slice(0, 1000)}`
                });
                output.set("noChangeSample", "[]");
                return;
            }
            ...
            const ROUTER_VERSION =
                "V2.60.0 SOURCE-WATCH-DRIVEN REGISTRY-CONTRACT PIPELINE MONITOR + DYNAMIC ADAPTER ROUTING + QUEUE ROUTER";
            ...
            function queueMateriality(delta) {
                ...
                if (delta.classification === "SOURCE DISAPPEARANCE") {
                    ...
                    if (
                        ...
                        existingPhase.includes("approved")
                    ) return "High";
                    return "Medium";
                }
            ...
            if (!queueWriteReady) {
                ...
                await sourceWatchTable.updateRecordAsync(sourceWatchRecord.id, {
                    [swf.lastChecked.id]: holdDate,
                    [swf.nextCheck.id]: retryDate,
                    [swf.stateNotes.id]:
                        `${ROUTER_VERSION}: RECONCILIATION HOLD - ${sourceRows.length} official ${companyName} programmes parsed via ${adapterProfile}; ` +
                        `${confirmed.length} confirmed matches, ${laterStagePreserved.length} later-stage preserved, ` +
                        `${newProgrammes.length} new programmes, ${sourceDisappearances.length} source disappearances, ` +
                        `${matchReview.length} identity/reconciliation review item(s). No queue or Portfolio writes applied. ` +
                        `Retry scheduled ${retryDate} after mapping/reconciliation review.`
                });
                ...
                output.set("genericQueueRouterSummary", JSON.stringify({
                    ...
                    matchReviewRequired: matchReview.length,
                    retryDate,
                    status: "RECONCILIATION HOLD - iteration isolated; remaining Source Watch records continue",
                    masterDataWrites: 0
                }, null, 2));
