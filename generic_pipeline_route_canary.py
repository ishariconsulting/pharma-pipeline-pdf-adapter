"""Read-only canary for the hardened semantic PDF pipeline table adapter."""
import asyncio, json
...
async def one(company,url):
    try:
        ...
        payload={
          ...
          "sampleRows":r.rows[:16],
          "masterWrites":0,
        }
    except Exception as exc:
        payload={"company":company,"readyForDiscovery":False,"error":f"{type(exc).__name__}: {exc}","masterWrites":0}
    print("GENERIC_PDF_CANARY_V2 "+json.dumps(payload,ensure_ascii=False),flush=True)
