"""Private-log export for AstraZeneca Portfolio Discovery staging QA.
...
Portfolio regression snapshot, then emits only actionable/review candidate JSON
into Render logs. The log payload is used to verify/idempotently stage candidates
in the dedicated Airtable Portfolio Discovery Candidates table. It never calls
Airtable and never mutates Portfolio, RPC, MRS, Queue, or existing automations.
"""
...
EXPORT_VERSION = "V1.0 ASTRAZENECA DISCOVERY STAGING EXPORT - READ ONLY"
STAGE_CLASSES = {
    "NEW ASSET",
    "NEW INDICATION",
    "POSSIBLE DUPLICATE",
...
def _compact(candidate: Dict[str, Any]) -> Dict[str, Any]:
    """Keep the complete staging contract while excluding no-longer-needed noise."""
    return {
...
async def run_staging_export() -> Dict[str, Any]:
    ...
    # One candidate per line keeps Render log retrieval/pagination deterministic.
