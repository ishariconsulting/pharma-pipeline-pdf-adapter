/*
V3.71 — SOURCE WATCH ADAPTER BINDING VALIDATION RUNNER + STATIC XLSX PIPELINE QA
Supported adapters: GENERIC_HTML_CONTENT_V1 + PIPELINE_GENERIC_HTML_V1 + PIPELINE_GENERIC_XLSX_V1
...
async function writeRun({
    ...
    return await validationRuns.createRecordAsync({
        ...
        [VF.runNotes]:
            "V3.71 source-specific Adapter Binding QA. Adds reusable semantic XLSX pipeline validation while preserving V3.70 HTML behaviour. XLSX validation is source-structure based and does not require an existing Portfolio alias. Uses Reconciliation validation type for compatibility with existing select options. No master intelligence writes.",
    });
}
...
if (adapterName === "PIPELINE_GENERIC_XLSX_V1" && fetchMethod !== "PDF / Document") {
    issues.push(`PIPELINE_GENERIC_XLSX_V1 requires Fetch Method PDF / Document; current=${fetchMethod || "blank"}.`);
}
if (adapterName !== "PIPELINE_GENERIC_XLSX_V1" && fetchMethod !== "HTML Page") {
